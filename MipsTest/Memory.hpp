...
void LoadFromPool(int *value, int pos, int size) {
    memcpy(value, pool + pos, size);
}
void StoreToPool(int *value, int pos, int size) {
    memcpy(pool + pos, value, size);
}
